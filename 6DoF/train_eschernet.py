...
def main(args):
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            # Move only necessary items to CUDA
            # if torch.cuda.is_available():
            #     batch["image_target"] = batch["image_target"].cuda()
            #     batch["image_input"] = batch["image_input"].cuda()
            #     batch["pose_in"] = batch["pose_in"].cuda()
            #     batch["pose_out"] = batch["pose_out"].cuda()
            #     batch["pose_in_inv"] = batch["pose_in_inv"].cuda()
            #     batch["pose_out_inv"] = batch["pose_out_inv"].cuda()
